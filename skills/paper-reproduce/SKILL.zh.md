...
如果你即将在任何描述文档或笔记里写下以下任何词语——**cannot**、**not available**、**too complex**、**not feasible**、**weeks of effort**、**not reproduced**——在写下之前必须先调用 `declare-barrier`。未完成 `declare-barrier` 前不得写下这些词。
...
6. **编译**：`compile_arguments` — 在进入阶段 2 之前验证逻辑链是否严密。修复所有标记的问题。
...
1. `compile_arguments` — 在汇报结果前，验证并修复所有未解决的论证链问题。
2. `get_stats`——查看整体进度
3. `get_argument(claim_id)`——逐一检查每个 Claim 的完整论证链
4. 检查孤立节点：没有 Warrant 的 Claim、没有 Ground 的 Warrant
5. 确认所有 Claim 状态已合理裁定
